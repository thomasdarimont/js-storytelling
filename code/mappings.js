...
class Mappings {
  ...
  load() {
    ...
    // Line: "motive--motion"
    // TotalVotesCast: "4"
    // VotesForThisColour: "2"

    for (let i = 0; i < this.senslevel.length; i++) {
      let sensEntry = this.senslevel[i];

      let word = sensEntry.Line.split("--")[0];
      let wordInfo = this.wordIndex[word];
      if (wordInfo) {
        Object.assign(wordInfo, sensEntry);
      }
    }
